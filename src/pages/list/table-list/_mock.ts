...
function genList(current: number, pageSize: number) {
...
function getRule(req: Request, res: Response, u: string) {
  ...
  if (params.pageSize)
    finalPageSize = Number.parseInt(`${params.pageSize}`, 10)
  ...
  const result = {
    ...
    current: Number.parseInt(`${params.currentPage}`, 10) || 1,
  }
...
function postRule(req: Request, res: Response, u: string, b: Request) {
  ...
  switch (req.method) {
    ...
    case 'DELETE':
      tableListDataSource = tableListDataSource.filter(item => !key.includes(item.key))
      break
    case 'POST':
      (() => {
